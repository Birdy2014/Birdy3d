...
namespace Birdy3d {
    ...
    void ScrollView::arrange(glm::vec2 pos, glm::vec2 size) {
        ...
        glm::vec2 content_size = glm::vec2(glm::max(minsize.x, size.x), glm::max(minsize.y, size.y));
        ...
        for (const auto& s : m_shapes) {
            s->parent_size(content_size);
        }

        if (m_layout && m_children_visible)
            m_layout->arrange(m_children, pos + glm::vec2(m_padding[0], m_padding[2]) + m_scroll_offset + pos_offset, content_size - glm::vec2(m_padding[0] + m_padding[1], m_padding[2] + m_padding[3]));
