...
#include "api/FPPlayerController.hpp"
...
int main() {
	if (!Engine::init("Birdy3d", 800, 600)) {
		return -1;
	}
	Input::init();
	...
	obj->addComponent(new Model(std::string(std::filesystem::current_path()) + std::string("/testObjects/cube.obj"), false, glm::vec4(1.0f, 1.0f, 1.0f, 1.0f), 16, glm::vec3(0.0f, 0.0f, 0.0f)));
	GameObject *obj2 = new GameObject(&geometryShader, glm::vec3(0.0f, -2.0f, 0.0f), glm::vec3(0.0f), glm::vec3(10.0f, 1.0f, 10.0f));
	obj2->addComponent(new Model(std::string(std::filesystem::current_path()) + std::string("/testObjects/cube.obj"), false, glm::vec4(1.0f, 1.0f, 1.0f, 1.0f), 16, glm::vec3(0.0f, 0.0f, 0.0f)));
	obj->addChild(obj2);
	scene->addChild(obj);
